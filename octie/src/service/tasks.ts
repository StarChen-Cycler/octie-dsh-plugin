...
function readNotesFile(notesFile: string): string {
 ...
 */
export async function updateTaskWithPropagation(
  projectPath: string,
  id: string,
  ...
  if ((patch.addNeedFix ?? []).length > 0) {
    ...
    for (const nf of patch.addNeedFix!) {
      task.addNeedFix(nf.text, { file_path: nf.file, source: source as 'review' | 'runtime' | 'regression' });
    }
    updated = true;
  }
  ...
  if (patch.blockers) {
    ...
    const blockerId: unknown = patch.blockers.id;
    if (typeof blockerId !== 'string' || blockerId.length === 0) {
      ...
      throw new CliPreparationError(
        `blockers accepts only ONE task ID string; received ${received}. To add multiple blockers, call once per blocker.`,
        ['Example: octie update abc123 --blockers def456 --dependency-explanation "Needs the API spec from def456"']
      );
    }
  ...
  if (!updated) throw new Error('No updates specified');
...
export async function approveTaskWithPropagation(
  projectPath: string,
  id: string,
): Promise<{ task: TaskProjection; propagatedCount: number }> {
  const storage = new TaskStorage({ projectDir: projectPath });
  const graph = await storage.load();
  const task = graph.getNodeByIdOrPrefix(id);
