...
export function addTaskCreationOptions(command) {
    return command
        .addOption(new Option('--title <string>', 'Task title (max 200 chars). Must contain action verb')
        .env('OCTIE_TASK_TITLE')
        .makeOptionMandatory(true))
        .addOption(new Option('--description <string>', 'Detailed task description (min 50 chars, max 10000)')
        .env('OCTIE_TASK_DESCRIPTION')
        .makeOptionMandatory(true))
        .addOption(new Option('--success-criterion <text>', 'Quantitative success criterion (can be specified multiple times)')
        .argParser((value, previous) => [...(previous || []), value])
        .env('OCTIE_SUCCESS_CRITERION')
        .makeOptionMandatory(true))
        .addOption(new Option('--deliverable <text>', 'Specific output expected (can be specified multiple times)')
        .argParser((value, previous) => [...(previous || []), value])
        ...
        .option('-d, --dependency-explanation <text>', 'Explanatory text: WHY this task depends on its blockers (required if --blockers is set)')
        .addOption(new Option('--dependencies <text>')
        .hideHelp())
        .addOption(new Option('-f, --related-files <paths>', 'File paths relevant to task (can be specified multiple times or comma-separated)').argParser((value, previous) => {
        const items = value.includes(',') ? value.split(',').map(item => item.trim()) : [value.trim()];
        return [...(previous || []), ...items.filter(Boolean)];
    }))
        .addOption(new Option('-c, --c7-verified <library:notes>', 'C7 library verification (format: library-id or library-id:notes, can be specified multiple times)').argParser((value, previous) => [...(previous || []), value]))
        .addOption(new Option('-n, --notes <text>', 'Additional context or comments (can be specified multiple times)').argParser((value, previous) => [...(previous || []), value]))
...
export function displayAtomicTaskPolicy() {
    console.log('');
