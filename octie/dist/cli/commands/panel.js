...
const ALL_STATUSES = ['ready', 'in_progress', 'in_review', 'completed', 'blocked'];
/**
 * Aggregate tasks from a loaded graph into a panel summary.
 */
function summarizeGraph(graph, name, projectPath, isRoot) {
 ...
 */
function discoverSubprojects(projectPath) {
...
function renderMarkdown(summaries) {
    const lines = ['# Octie Panel Summary\n'];
    ...
    for (const s of summaries) {
        ...
        lines.push(`| ${name} | ${s.total} (${statusList}) | ${s.completedPct}% | ${s.updatedAt} | ${s.goal} |`);
    }
    return lines.join('\n');
}
/**
    ...
    try {
        ...
        switch (format) {
            ...
            default: {
                ...
                console.log(chalk.gray(`Total: ${summaries.length} panel${summaries.length !== 1 ? 's' : ''} (${totalTasks} task${totalTasks !== 1 ? 's' : ''})`));
                break;
            }
        }
