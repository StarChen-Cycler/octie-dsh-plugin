...
function formatHistoryMarkdown(entries) {
    if (entries.length === 0) {
        return '# Snapshot History (0)\n';
    }
    const lines = ['# Snapshot History', '', `Snapshots: ${entries.length}`, ''];
    for (const entry of entries) {
...
function outputHistory(entries, format) {
    switch (format) {
        case 'json':
...
`)
    .action(async (snapshotId, options, command) => {
    try {
        const globalOpts = command.parent?.parent?.opts() || {};
        const projectPath = await getProjectPath(globalOpts.project);
        const storage = new TaskStorage({ projectDir: projectPath });
