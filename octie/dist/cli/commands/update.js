...
export const updateCommand = new Command('update')
    ...
    .addOption(new Option('--priority <priority>', 'Task priority')
    .choices(['top', 'second', 'later']))
    .addOption(new Option('--add-deliverable <text>', 'Add a deliverable (can be specified multiple times)')
    ...
    .addOption(new Option('--add-success-criterion <text>', 'Add a success criterion (can be specified multiple times)')
    .argParser((value, previous) => [...(previous || []), value]))
    ...
    .addOption(new Option('--add-related-file <path>', 'Add a related file path (can be specified multiple times)')
    .argParser((value, previous) => [...(previous || []), value]))
    .option('--remove-related-file <path>', 'Remove a related file path')
    .addOption(new Option('--verify-c7 <library:notes>', 'Add C7 library verification (can be specified multiple times, format: library-id or library-id:notes)')
    .argParser((value, previous) => [...(previous || []), value]))
    .option('--remove-c7-verified <library>', 'Remove a C7 verification by library ID')
    .addOption(new Option('--add-need-fix <text>', 'Add a need_fix item (blocking issue, can be specified multiple times). Use --need-fix-source to specify source.')
    .argParser((value, previous) => [...(previous || []), value]))
    .addOption(new Option('--need-fix-source <source>', 'Source of need_fix')
    ...
    .addOption(new Option('--notes <text>', 'Append to notes (can be specified multiple times)')
    .argParser((value, previous) => [...(previous || []), value])
    .default([]))
    ...
    try {
        ...
        const patch = {
            ...
            addNeedFix: (options.addNeedFix || []).map((text) => ({
                text,
