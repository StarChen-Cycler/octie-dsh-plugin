 ...
 */
export function walkUpstream(graph, taskId, depth = 0, visited = new Set()) {
 ...
 * @internal exported for testing
 */
export function walkDownstream(graph, taskId, depth = 0, visited = new Set()) {
    if (visited.has(taskId))
        return [];
    visited.add(taskId);
...
`)
    ...
    try {
        ...
        if (showUpstream)
            walkUpstream(graph, task.id).forEach(t => allIds.add(t.id));
        if (showDownstream)
            walkDownstream(graph, task.id).forEach(t => allIds.add(t.id));
