 ...
 */
export function formatTaskDetailTable(task, fields) {
    ...
    const infoRows = [];
    if (show('status'))
        infoRows.push([chalk.gray('Status:'), formatStatus(task.status)]);
    if (show('priority'))
        infoRows.push([chalk.gray('Priority:'), formatPriority(task.priority)]);
    if (show('created_at'))
        infoRows.push([chalk.gray('Created:'), task.created_at]);
    if (show('updated_at'))
        infoRows.push([chalk.gray('Updated:'), task.updated_at]);
    if (show('completed_at') && task.completed_at)
        infoRows.push([chalk.gray('Completed:'), task.completed_at]);
