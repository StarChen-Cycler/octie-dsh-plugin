 ...
 */
const QUANTITATIVE_ANCHOR_PATTERNS = [
 ...
 */
function hasQuantitativeAnchor(text) {
 ...
 */
function findSubjectiveWord(tokens) {
...
export function validateAtomicTask(taskData) {
    ...
    if (!isUnicodeTitle && !hasActionVerb) {
        violations.push(`Title should contain an action verb. Accepted verbs (${ACTION_VERBS.length}): ${ACTION_VERBS.join(', ')}`);
    }
    ...
    if (taskData.deliverables.length > 10) {
        violations.push(`Too many deliverables (${taskData.deliverables.length} > 10). This suggests the task is too large and should be split into smaller, focused tasks.`);
    }
...
export class TaskNode {
     ...
     */
    completeCriterion(criterionId, evidence) {
    ...
    addDeliverable(deliverable) {
        if (this.deliverables.length >= 10) {
            throw new ValidationError('Too many deliverables (max 10). Split into smaller tasks.', 'deliverables');
        }
