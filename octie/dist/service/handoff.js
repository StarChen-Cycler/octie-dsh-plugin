...
import { CliPreparationError, executeProjectInit, executeTaskCreation, preflightProjectInit, preflightTaskCreation, toCreateOptions, } from './engine.js';
...
function rollbackChildProject(childProjectPath) {
    ...
    try {
        ...
        rollbackFailures.push(`directory cleanup failed for ${childProjectPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    if (rollbackFailures.length > 0) {
        throw new Error(`rollback cleanup incomplete\n${rollbackFailures.map(m => `- ${m}`).join('\n')}`);
    }
}
export async function createHandoff(projectPath, input) {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Original failure: ${err instanceof Error ? err.message : String(err)}\n` +
