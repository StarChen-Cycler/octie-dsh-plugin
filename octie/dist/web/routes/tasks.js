...
const TaskQuerySchema = z.object({
    ...
    limit: z.string().transform(val => val ? parseInt(val, 10) : undefined).pipe(z.number().int().positive().max(1000).optional()).optional(),
    offset: z.string().transform(val => val ? parseInt(val, 10) : undefined).pipe(z.number().int().nonnegative().optional()).optional(),
});
/**
 ...
 */
export function registerTaskRoutes(router, getGraph, saveGraph, broadcastRefresh) {
     ...
     */
    async function saveProject(projectPath, graph) {
        if (!projectPath) {
    ...
    async function getProjectGraph(projectPath) {
        ...
        // (CLI can modify any project, fs.watch only watches the server's project,
        // so caching cross-project data leads to stale reads)
        try {
     ...
     */
    router.get('/api/tasks/:id', asyncHandler(async (req, res) => {
        const projectPath = getProjectPath(req);
        const graph = await getProjectGraph(projectPath);
        if (!graph) {
            return sendError(res, 'GRAPH_NOT_LOADED', projectPath ? `Project not found: ${projectPath}` : 'Graph not loaded', 500);
    ...
    router.post('/api/tasks', asyncHandler(async (req, res) => {
        ...
        try {
            ...
            if (data.blockers.length > 0 && !data.dependencies) {
                return sendError(res, 'TWIN_VALIDATION_ERROR', 'When blockers are provided, dependencies explanation text is also required', 400);
            }
            if (data.dependencies && data.blockers.length === 0) {
                return sendError(res, 'TWIN_VALIDATION_ERROR', 'When dependencies explanation is provided, blockers are also required', 400);
            }
        ...
        try {
            ...
            if (data.block !== undefined) {
                ...
                if (!data.dependencyExplanation) {
                    return sendError(res, 'TWIN_VALIDATION_ERROR', 'When adding a blocker, dependencyExplanation is required', 400);
                }
            ...
            if (data.dependencyExplanation !== undefined && data.block === undefined) {
                if (task.blockers.length === 0) {
                    return sendError(res, 'TWIN_VALIDATION_ERROR', 'Cannot set dependencies explanation without blockers', 400);
                }
     ...
     */
    router.post('/api/tasks/:id/merge', asyncHandler(async (req, res) => {
        const projectPath = getProjectPath(req);
        const graph = await getProjectGraph(projectPath);
        if (!graph) {
            return sendError(res, 'GRAPH_NOT_LOADED', projectPath ? `Project not found: ${projectPath}` : 'Graph not loaded', 500);
     ...
     */
    router.post('/api/cache/invalidate', asyncHandler(async (req, res) => {
        const projectPath = getProjectPath(req);
